...
"""
Offline hierarchical-distance utilities.
No files, no network – works for any label list.
distance = 0  (same label)
         = 1  (share first token → usually family)
...
def expected_distance(logits, target, D):
    ...
    P = torch.softmax(logits, 1)
    Dt = torch.from_numpy(D[target.cpu()]).to(logits)
    # scale to [0,1] so magnitude matches CE/focal (~1.0)
    return ((P * Dt).sum(1) / Dt.max()).mean()
